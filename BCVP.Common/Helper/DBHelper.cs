...
namespace BCVP.Common.Helper
{
    public class DBHelper
    {
        ...
        public static string getCountSql(string strSql)
        {
            ...
            {
                //int indexofCount = strSql.ToUpper().IndexOf("FROM");
                //截取
                //return $@"SELECT COUNT(*) ROWTOLSNUM " + strSql.ToUpper().Substring(indexofCount, strSql.Length - indexofCount);
                return $@"SELECT COUNT(*) ROWTOLSNUM  FROM( " + strSql + ")";
            }
            return null;
        }
        ...
        /// <returns></returns>
        public static string getDataPage(string strSql, int? page, int? limit)
        {
            return $@"SELECT * FROM (SELECT ROWNUM AS ROWNO, R.* FROM ({strSql}) R WHERE ROWNUM <= {page} * {limit}) TABLE_ALIAS
