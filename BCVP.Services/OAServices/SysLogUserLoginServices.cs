...
namespace BCVP.Services.OAServices
{
 ...
 WHERE (L.LOGIN_MESSAGE LIKE '%{keyWord}%' OR U.USERNAME LIKE '%{keyWord}%' OR U.USERACCOUNT LIKE '%{keyWord}%')
   {sqlWhere} ORDER BY {field} {order}";
        ...
        /// <returns></returns>
        public async Task WriteSystemLog(JsonResponse jsonResponse)
        {
            try
            {
                SysLogUserLogin LogModel = new SysLogUserLogin();
                LogModel.Login_Type = LoginType.Login;
                ...
                //LogModel.Login_CreateTime = Convert.ToDateTime(DateTime.Now.ToString());
