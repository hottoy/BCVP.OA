...
## 下载
Github（国际） 下载 [https://github.com/anjoy8/BCVP](https://github.com/anjoy8/BCVP)  
    
Gitee（国内） 下载 [https://gitee.com/laozhangIsPhi/BCVP](https://gitee.com/laozhangIsPhi/BCVP)  


## 编译与运行
1、拿到项目后，双击 `BCVP.sln` 解决方案；  
2、首先 `F6` 编译，看是否有错误；  
...
1、打开 `BCVP` 项目下的 `appsettings.json` 文件；  
...
## 如何配置项目端口号
1、在 `BCVP` 层下的 `program.cs` 文件中，将 `9291`端口，修改为自己想要的端口号；    
...
## 新增实体模块后如何迁移到数据库
1、在 `BCVP.Model` 项目目录下的 `Seed` 文件夹下，找到 `DBSeed` 类；  
...
## 新增数据库表，如何反向生成四层文件
1、可以通过 `T4` 模板来生成，在 `BCVP.FrameWork` 层，使用方法: [9757999.html](https://www.cnblogs.com/laozhang-is-phi/p/9757999.html#autoid-4-3-0) ；  
...
> 3、就会在对应的层内，看到新文件，比如：BCVP.Model/Model_NEW   
...
## 发布与部署
1、双击项目根目录下的 `BCVP.Publish.bat`批处理文件；  
